...
class RAGFlowPdfParser:
    def __init__(self, **kwargs):
        ...
        try:
            ...
            # --- 强力修改版：彻底断网加载 XGBoost ---
        model_dir = get_deepdoc_model_directory()
        xgb_model_path = os.path.join(model_dir, "updown_concat_xgb.model")

        print(f"🔗 [模型加载] 正在加载段落合并模型: {xgb_model_path}")

        # 强制执行加载，不再尝试 snapshot_download
        self.updown_cnt_mdl.load_model(xgb_model_path)
