...
def load_model(model_dir, nm, device_id: int | None = None):
    ...
    # 2. 【核心优化】先查缓存，如果命中，0秒返回，不消耗任何多余计算
    if model_cached_tag in loaded_models:
        return loaded_models[model_cached_tag]

    # --- 如果没命中，才开始昂贵的初始化工作 ---
    if not os.path.exists(model_file_path):
        raise ValueError(f"找不到模型文件: {model_file_path}")
    ...
    if 'CUDAExecutionProvider' in available_providers:
        ...
        # 用 print 替代 logging，能让你在控制台最直观地看到飞快的加速日志
        print(f"🚀 [GPU模式] 模型 {nm} 已加载至显卡 Device {provider_device_id}")
    else:
        sess = ort.InferenceSession(
            model_file_path,
            options=options,
        ...
        print(f"🐢 [CPU模式] 未发现可用GPU，模型 {nm} 正在慢速运行")
...
class OCR:
    def __init__(self, model_dir=None):
        ...
        print(f"🔍 [GPU模式] OCR 引擎正在加载本地权重: {model_dir}")

        # 2. 检查路径是否存在
        if not os.path.exists(model_dir):
            print(f"❌ 错误：在 {model_dir} 未找到模型文件，请检查上传路径！")

        # 3. 直接初始化检测器和识别器，不再使用 try...except 包裹
