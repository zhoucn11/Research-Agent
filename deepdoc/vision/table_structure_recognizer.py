...
# 添加以下伪造代码，切断与 infinity 后端的联系
class MockTokenizer:
    def tokenize(self, text):
        # 把返回列表改成返回 "空格隔开的字符串"，这样后面调 .split() 就不会报错了！
        return " ".join(list(text)) if text else ""

    def fine_grained_tokenize(self, text):
        return text.split() if text else []

    # 顺便补一个 tag 方法，防止它后续做词性标注时找不到
    def tag(self, text):
        return "n"
...
# 如果有，同样注释掉，并换成我们之前写的 MockSettings：
class MockSettings:
    PARALLEL_DEVICES = 0
settings = MockSettings()
...
class TableStructureRecognizer(Recognizer):
    ...
    def __init__(self):
        ...
        # 2. 打印一下，方便调试
        print(f"📊 [GPU模式] 正在从本地加载表格解析模型 (TSR)...")

        # 3. 这里的 "tsr" 是模型名称，不能改；model_dir 是路径
